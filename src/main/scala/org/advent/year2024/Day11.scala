...
object Day11 extends Problem[List[Long]](2024, 11) {
  ...
  private def blink(stone: Long, count: Int, map: mutable.Map[(Long, Long), Long] = mutable.Map.empty): Long =
    if (count == 0) { map((stone, count)) = 1; 1 } else if (map.contains((stone, count))) map((stone, count)) else {
      map((stone, count)) = if (stone == 0) blink(1, count - 1, map) else if (stone.length % 2 == 0) { val (left, right) =
        stone.split; blink(left, count - 1, map) + blink(right, count - 1, map) } else blink(stone * 2024, count - 1, map)
      map((stone, count))
    }
